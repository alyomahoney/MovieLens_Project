...
# setwd("...")
source("cf_scripts/cf_algorithm.R")
source("cf_scripts/similarity_measures.R")
...
evaluate_cf <- function(ratings_matrix, number_of_folds, ...) {
  ...
  all_predictions@x[all_predictions@x < 0.5] <- 0.5
...
start <- Sys.time()
res <- evaluate_cf(ratings_matrix, number_of_folds = 10, alg_method = "ubcf", normalization = TRUE, 
                   similarity_metric = cal_cos, k = 100, make_positive_similarities = FALSE, 
                   rowchunk_size = 20000, columnchunk_size = 6000)
end <- Sys.time()
print(end - start)
