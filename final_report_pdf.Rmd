...
![Histogram of all movie ratings in edx. The red dashed line represents the mean movie rating.](rmd_files/plots/hist_of_ratings.png)
...
![Scatterplot of mean movie rating for each release year. Note - the grey shaded area does not account for the size or spread of the sample from which the mean ratings were calculated.](rmd_files/plots/release_year.png) 
...
![Scatterplot of mean movie rating for each year of review. Note - the grey shaded area does not account for the size or spread of the sample from which the mean rating was calculated.](rmd_files/plots/year_rated.png) 
...
![Scatterplot of mean movie rating for each month of review. Note - the grey shaded area does not account for the size or spread of the sample from which the mean rating was calculated.](rmd_files/plots/month_rated.png) 
...
![Mean rating for each genre (random sample of 100,000 observations).](rmd_files/plots/by_genre.png)
...
```{r rmse_1, echo = FALSE}
kable(rmse_results %>% slice(1), caption = "RMSE result after first model.") %>% row_spec(0,bold=TRUE) %>%
...
![Histogram of mean ratings for each movie](rmd_files/plots/movie_hist.png)
...
kable(rmse_results %>% slice(1:2), caption = "RMSE results after second model.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
![Histogram of mean ratings for each user.](rmd_files/plots/user_hist.png)
...
kable(rmse_results %>% slice(1:3), caption = "RMSE results after third model.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
kable(top_five, col.names = c("Title", "$\\beta_m$", "Number of ratings"), escape = FALSE, caption = "Top five movies according to \"Movie Bias\" model.") %>% column_spec(1, width = "2.5in") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")

kable(bottom_five, col.names = c("Title", "$\\beta_m$", "Number of ratings"), escape = FALSE, caption = "Worst five movies according to \"Movie Bias\" model.") %>% column_spec(1, width = "2.5in") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
![Scatterplot of RMSE for each $\lambda$ (5-fold cross-validation). The optimal value of $\lambda=2.3$ is highlighted.](rmd_files/plots/lambdas_m.png)
...
kable(top_five_reg, col.names = c("Title", "$\\beta_m$", "Number of ratings"), escape = FALSE, caption = "Top five movies according to \"Regularised Movie Bias\" model.") %>% column_spec(1, width = "2.5in") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")

kable(bottom_five_reg, col.names = c("Title", "$\\beta_m$", "Number of ratings"), escape = FALSE, caption = "Worst five movies according to \"Regularised Movie Bias\" model.") %>% column_spec(1, width = "2.5in") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
kable(rmse_results %>% slice(1:4), caption = "RMSE results after fourth model.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
![Scatterplot of RMSE for each $\lambda$ (5-fold cross-validation). The optimal value of $\lambda=5.1$ is highlighted.](rmd_files/plots/lambdas_u.png)

```{r rmse_5, echo = FALSE}
kable(rmse_results %>% slice(1:5), caption = "RMSE results after fifth model.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
![Nikolić's illustration of splitting the matrix into chunks.](rmd_files/chunks.png)
...
kable(data.frame(Parameter = c("k", "Row chunk size", "Column chunk size"),
      ...
      caption = "Parameters used for UBCF.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
$$\hat{r}_{u,m} = \left\{\begin{array}{ll}\frac{\sum_{U\in S}sim(u, U)r_{U,m}}{\sum_{U\in S}|sim(u, U)|}  & \mbox{if } S \neq \emptyset \\
...
```{r rmse_6, echo = FALSE}
kable(rmse_results %>% slice(1:6), caption = "RMSE results after sixth model.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
kable(data.frame(Parameter = c("k", "Row chunk size", "Column chunk size"),
      ...
      caption = "Parameters used for IBCF.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```

```{r rmse_7, echo = FALSE}
kable(rmse_results %>% slice(1:7), caption = "RMSE results after seventh model.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
![RMSE values for various instances of p](rmd_files/plots/ens_cv.png)


```{r rmse_8, echo = FALSE}
kable(rmse_results %>% slice(1:8), caption = "RMSE results after eighth model.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
...
$$\hat{r}_{u,m}^{UBCF} = \left\{\begin{array}{ll}\frac{\sum_{U\in S}sim(u, U)r_{U,m}}{\sum_{U\in S}|sim(u, U)|}  & \mbox{if } S \neq \emptyset \\
		\mu + \beta_m+\beta_u & \mbox{otherwise,} 
	\end{array}
\right.$$

$$\hat{r}_{u,m}^{IBCF} = \left\{\begin{array}{ll}\frac{\sum_{M\in T}sim(m, M)r_{u,M}}{\sum_{M\in T}|sim(m, M)|}  & \mbox{if } S \neq \emptyset \\
		\mu + \beta_m+\beta_u & \mbox{otherwise } 
	\end{array}
\right.$$
where
...
kable(final_results, caption = "Table 15: RMSE of final model.") %>% row_spec(0,bold=TRUE) %>%
  kable_styling(latex_options = "hold_position")
```
