...
load("rmd_files/rmse_results.rda")
load("rmd_files/rmse_values.rda")
load("rmd_files/reg_motivation.rda")
load("rmd_files/opt_lambdas.rda")
load("rmd_files/predictions.rda")
load("rmd_files/edx_str.rda")
...
![Histogram of all movie ratings in training set. The red dashed line represents the mean movie rating.](plots/hist_of_ratings.png)
...
![Scatterplot of mean movie rating for each release year. Note - the grey shaded area does not account for the size of the sample from which the mean rating was calculated.](plots/release_year.png)
...
![Scatterplot of mean movie rating for each year of review. Note - the grey shaded area does not account for the size of the sample from which the mean rating was calculated.](plots/year_rated.png)
...
![Scatterplot of mean movie rating for each month of review. Note - the grey shaded area does not account for the size of the sample from which the mean rating was calculated.](plots/month_rated.png)
...
![Mean rating for each genre (random sample of 100,000 observations).](plots/by_genre.png)
...
![Histogram of mean ratings for each movie](plots/movie_hist.png)
...
![Histogram of mean ratings for each user.](plots/user_hist.png)
...
![Scatterplot of RMSE for each $\lambda$ (5-fold cross-validation). The optimal value of $\lambda=2.3$ is highlighted.](plots/lambdas_m.png)
...
![FScatterplot of RMSE for each $\lambda$ (5-fold cross-validation). The optimal value of $\lambda=5.1$ is highlighted.](plots/lambdas_u.png)

Again, this model is only a slight improvement over the [Movie and User Bias] model. The RMSE is reduced to `r round(rmse_5, 4)`.
...
![Nikolić's illustration of splitting the matrix into chunks.](images/chunks.png)
